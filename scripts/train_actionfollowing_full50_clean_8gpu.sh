...
[[ -f "$AFD_ROOT/demo_clean_zed2i_visible/adjust_bottle/meta/info.json" ]] || {
  echo "ERROR: canonical ActionFollowingData root is not mounted: $AFD_ROOT" >&2
  exit 1
}
...
torchrun --nproc_per_node=8 --master_port="${MASTER_PORT:-12347}" \
  ...
  job.name=cosmos_predict25_afd_full50_clean_rot6d20_a32_bs16_40k \
