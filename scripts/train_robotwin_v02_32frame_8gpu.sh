#!/bin/bash
# Action-Conditioned 后训练 V0.2 —— RoboTwin 双臂 14D, 生成 32 帧, 224×224
# 基于现有 AC 14D experiment 做参数 override：
#   - num_action_per_chunk: 12 → 32（生成帧数 = chunk size）
#   - video_size: [256, 320] → [224, 224]
#   - fps_downsample_ratio: 10 → 1（24 fps 不抽帧）
#   - state_t: 4 → 9（= 1 + 32/4, tokenizer 时间压缩 4×）
#   - max_iter: 150k → 100k
...
torchrun --nproc_per_node=8 --master_port=12343 \
  -m scripts.train \
  --config=cosmos_predict2/_src/predict2/action/configs/action_conditioned/config.py \
  -- experiment=robotwin_14d_8gpu \
  job.name=robotwin_14d_v02_32f_224_8gpu \
  trainer.max_iter=100000 \
  model.config.state_t=9 \
  model.config.net.num_action_per_chunk=32 \
  dataloader_train.sampler.dataset.num_action_per_chunk=32 \
  dataloader_train.sampler.dataset.fps_downsample_ratio=1 \
  'dataloader_train.sampler.dataset.video_size=[224,224]' \
  dataloader_train.dataset.num_action_per_chunk=32 \
